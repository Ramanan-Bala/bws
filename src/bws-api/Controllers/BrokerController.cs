...
namespace bws_api.Controllers
{
    ...
    public class BrokerController : ControllerBase
    {
        ...
        public IActionResult Post(Broker broker)
        {
            using (var con = Connect())
            {
                broker.Id = con.QuerySingle<int>("INSERT INTO brokers (BrokerName,AddressLine1,AddressLine2,City,ContactNumber,Note)" +
                "VALUES (@brokerName,@addressLine1,@addressLine2,@city,@contactNumber,@note);SELECT last_insert_id()", broker);
        ...
        public IActionResult Put(int id, Broker broker)
        {
            ...
            using (var con = Connect())
            {
                con.Execute("UPDATE brokers SET BrokerName=@brokerName,AddressLine1=@addressLine1,AddressLine2=@addressLine2,City=@city,ContactNumber=@contactNumber,Note=@note" +
                " WHERE Id=@id", broker);
                return Ok(broker);
            }
        }
